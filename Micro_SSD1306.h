...
#ifndef _Micro_SSD1306_H_
#define _Micro_SSD1306_H_
...
// plain C interface for use with SDCC
  ...
  void Micro_SSD1306_clearDisplay(void);
  void Micro_SSD1306_invertDisplay(uint8_t i);
  void Micro_SSD1306_display();
...
//  void Micro_SSD1306_drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color);
//  void Micro_SSD1306_drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color);

//  inline void Micro_SSD1306_drawFastVLineInternal(int16_t x, int16_t y, int16_t h, uint16_t color) __attribute__((always_inline));
//  inline void Micro_SSD1306_drawFastHLineInternal(int16_t x, int16_t y, int16_t w, uint16_t color) __attribute__((always_inline));
...
#define XMicro_SSD1306Inst(instance,RST) inline \
        void instance##_inst(void){\
		Micro_SSD1306_init(RST);\
	}

#define XMicro_SSD1306Begin(instance) inline \
        void instance##_begin(uint8_t vccstate, uint8_t i2caddr, bool reset){\
		instance##_inst(); \
		Micro_SSD1306_begin(vccstate,i2caddr,reset);\
	}

#define XMicro_SSD1306Methods(instance) \
	XMicro_SSD1306Begin	(instance)\
	\
	X2Method0	(Micro_SSD1306,instance,clearDisplay) \
	X2Method1	(Micro_SSD1306,instance,invertDisplay,uint8_t) \
	X2Method0	(Micro_SSD1306,instance,display) \
	X2Method2	(Micro_SSD1306,instance,startscrollright,uint8_t,uint8_t) \
	X2Method2	(Micro_SSD1306,instance,startscrollleft,uint8_t,uint8_t) \
	X2Method2	(Micro_SSD1306,instance,startscrolldiagright,uint8_t,uint8_t) \
	X2Method2	(Micro_SSD1306,instance,startscrolldiagleft,uint8_t,uint8_t) \
	X2Method0	(Micro_SSD1306,instance,stopscroll) \
	X2Method1	(Micro_SSD1306,instance,dim,boolean) \
  X2Method0 (Micro_SSD1306,instacne,initPages) \
  X2Method2 (Micro_SSD1306,instance,setCursor,uint8_t, uint8_t) \
  X2Method0 (Micro_SSD1306,instance,clearPages) \
  X2Method1 (Micro_SSD1306,instance,printChar,char) \
  X2Method1 (Micro_SSD1306,instance,printString,char*)
...
//
// Calling the 'XMicro_SSD1306Methos' macro defines all functions specific for this
...
#define Micro_SSD1306(instance,RST) \
	extern int 		Micro_SSD1306; \
	XMicro_SSD1306Inst	(instance,RST) \
	XMicro_SSD1306Methods	(instance)\
	extern int 		Micro_SSD1306

// not used (yet):
//	XPrintMethods		(Micro_SSD1306,instance)

#endif /* c interface */

#endif /* _Micro_SSD1306_H_ */
